...
export class UIRenderer {
  static draw(ctx: CanvasRenderingContext2D, state: DuelState): void {
    this.drawPlayerPanel(ctx, state, "p1", 24, 22, false);
    this.drawPlayerPanel(ctx, state, "p2", LOGICAL_WIDTH - 264, 22, true);
    ...
    if (state.winner) {
      ...
      ctx.textAlign = "center";
      ctx.fillText(`${state.winner.toUpperCase()} Victory`, LOGICAL_WIDTH / 2, LOGICAL_HEIGHT / 2);
      ctx.font = "16px sans-serif";
      ctx.fillStyle = "#a99b82";
      ctx.fillText("Reload or return to the menu for another duel.", LOGICAL_WIDTH / 2, LOGICAL_HEIGHT / 2 + 34);
    }
  }
  ...
  private static drawPlayerPanel(
    ...
    ctx.fillText(`${playerId.toUpperCase()} ${weapon.label}`, textX, y + 20);
    ...
    ctx.fillText(`${player.action} / angle ${player.weaponAngle.toFixed(1)}`, textX, y + 73);
  }
  ...
  private static drawBottomInfo(ctx: CanvasRenderingContext2D, state: DuelState): void {
    ...
    ctx.textAlign = "left";
    ctx.fillText(
      "Controls: Mouse aim, LMB attack, RMB guard, R parry, Q feint, F kick, E break, A/D move",
      36,
      LOGICAL_HEIGHT - 46,
    );
    ctx.fillText(
      "Remote fighter is driven by WebRTC input sync",
      36,
      LOGICAL_HEIGHT - 30,
    );
